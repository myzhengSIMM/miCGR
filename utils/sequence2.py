...
def get_candidate(mirna_sequence, mrna_sequence, cts_size, seed_match, up_stream=5, dn_stream=5):
...
def make_pair2(mrna_sequence):
    mrna_targets = [str(Seq(mrna_sequence[:40]).reverse_complement().transcribe())]
    posistions = [0]
    return  mrna_targets, posistions
...
def make_test_input_pair3(test_ifile,
    ...
    for i in range(len(query_ids)):
        ...
        if len(mrna_seq) < (cts_size + 2*5):
            continue
        target_seqs, locations = make_pair2(mrna_seq)
        n_pairs = len(locations)
        if n_pairs > 0:
            queries = [query_ids[i] for n in range(n_pairs)]
            query_seqs = [mirna_seq for n in range(n_pairs)]
            dataset['query_ids'].extend(queries)
            dataset['query_seqs'].extend(query_seqs)
