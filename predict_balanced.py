...
record_fout = open(os.path.join(save_odir, "records_outtest.csv"), 'w')
record_fout.write(','.join(['split', 'Accuracy', 'Sensitivity', 'Specificity', 'F1', 'PPV', 'NPV'])+'\n')
for seed_match in ["strict", "offset-9-mer-m7", "10-mer-m7", "10-mer-m6"]:
#for seed_match in ["strict"]:
    print("[{:s}]".format(seed_match))
    record_fout.write("{:s}\n".format(seed_match))
    for i in range(10):
        print("test_s{:d}".format(i))
        query_file = "balanced_data/randomLeveragedTestSplit_{:d}.csv".format(i)
        output_file = save_odir + "/test_result_{:s}_s{:d}.csv".format(seed_match, i)
        if kmer == 2:
        ...
        record_fout.write("test{:d},{:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}\n".format(
            i, acc, sensitivity, specificity, f1, ppv, npv))
        # break
    record_fout.write('\n')
    record_fout.write('\n')
    # break
